...
2. Copy `k8s/secret.example.yaml` to your deployment system and replace placeholder values.
3. Update `k8s/deployment.yaml` with the real image tag, real `NBLAUNCH_GALLERY_BASE_URL`, real `NBLAUNCH_GALLERY_DOWNLOAD_PATH_TEMPLATE`, real `NBLAUNCH_GALLERY_USER_AGENT` if needed, and any cluster-specific PVC name.
4. Apply the `nblaunch` Secret, Deployment, and Service alongside the matching Hub-side service registration managed in your JupyterHub repo.
5. Wait for the Deployment to become ready before routing user traffic.
...
5. Generate a signed launch URL with `scripts/generate_nblaunch_url.sh` and validate the expected redirect behavior in staging.

## Signed URL generation

Use `scripts/generate_nblaunch_url.sh` to generate a signed launch URL:
...
Run the standalone repo test suite from the repo root:
...
- `403 hub_home_lookup_forbidden`: Hub API rejected the nblaunch service identity.
- `502 hub_home_lookup_failed`: Hub API route, authorization exchange, or returned subpath is invalid.
...
3. Confirm the service token in Kubernetes matches the JupyterHub service registration in the Hub repo.
4. Confirm the mounted notebook root matches the home-subpath contract used by your Hub deployment.
5. Re-run `pytest` after any config edit.
